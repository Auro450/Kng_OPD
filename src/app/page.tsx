...
export default function Home() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const drRes = await fetch(`${"https://15-206-97-54.nip.io"}/api/doctors`);
        const drData = await drRes.json();
        if (Array.isArray(drData) && drData.length > 0) setDoctors(drData);

        const blogRes = await fetch(`${"https://15-206-97-54.nip.io"}/api/blog`);
        const blogData = await blogRes.json();
        if (Array.isArray(blogData) && blogData.length > 0) setBlogs([...ORIGINAL_BLOGS, ...blogData]);

        const reviewRes = await fetch(`${"https://15-206-97-54.nip.io"}/api/reviews`);
