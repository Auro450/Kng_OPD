...
export default function DiagnosticCentrePage() {
  ...
  useEffect(() => {
    async function fetchTests() {
      try {
        const res = await fetch(`${(process.env.NEXT_PUBLIC_API_URL || "https://15-206-97-54.nip.io")}/api/tests`);
        const data = await res.json();
  ...
  const handlePayment = async () => {
    ...
    try {
      const orderResponse = await fetch(`${(process.env.NEXT_PUBLIC_API_URL || "https://15-206-97-54.nip.io")}/api/create-order`, { 
        method: "POST",
      ...
      const options = {
        ...
        handler: async function (response: any) {
          const verifyRes = await fetch(`${(process.env.NEXT_PUBLIC_API_URL || "https://15-206-97-54.nip.io")}/api/verify-payment`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
  ...
  const submitCollection = async (dataToSubmit: any, selectedTestsList: string[], paymentId?: string) => {
    ...
    try {
      const res = await fetch(`${(process.env.NEXT_PUBLIC_API_URL || "https://15-206-97-54.nip.io")}/api/submit`, {
        method: "POST",
        body: formData,
      });
